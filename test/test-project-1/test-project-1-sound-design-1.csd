<CsoundSynthesizer>
<CsOptions>
-F /home/joel/mmw/test/test-project-1/test-project-1-sound-design-1.mid
</CsOptions>
<CsInstruments>
sr		  = 48000
kr 		  = 4800
...
;ivibdel   = 0.357
;kfrq 	  cpsmidib 1
;kenv 	  expsegr 0.1, idec, 0.01, 0.5, 0.001
;kindex    expon    2.856, 0.178, 0.178
;kvibenv	  linseg   0, ivibdel, 0.356, ivibdel, .1
;klfo	  oscil    kvibenv, kfrq*8, 1
;asig1 	  oscili  kenv*iamp, kfrq*1.428, 1 
;asig2	  oscili  kenv, kfrq*1.001, 1
;asig3	  oscili  kenv, kfrq*.999, 1
;asig4	  foscil  kenv, kfrq+klfo, 0.357, 1, kindex
;amix	  = (asig1+(asig2*0.1)+(asig3*0.1)+asig4*0.05)*0.35
;afilt	  tone amix, 9500
;
;	      outch $voice, afilt
;		  endin
...
;kfrq 	  cpsmidib 1
;kvibenv	  linseg   0, ivibdel, 0.356, ivibdel, .1
;klfo	  oscil    kvibenv, kfrq*0.15, 1
;kindex    expon    20.856, 2.178, 0.178
;kenv      linsegr  0.001, 0.357, iamp, 0.357, 0.001 
;
;asig1 	  oscili  kenv, kfrq, 1 
;asig2	  oscili  kenv, kfrq*1.001, 1
;asig3	  oscili  kenv, kfrq*.999, 1
;asig4	  foscil  kenv, kfrq+klfo, 0, 0, kindex
;
;amix	  = (asig1+(asig2*0.1)+(asig3*0.1)+asig4*0.05)*0.8525
;anoise	  rand kfrq
;afilt	  tone amix+((anoise*0.0000025)*klfo), 22000
;aout	  distort	afilt*kenv,kvibenv*1, gifn
;		  outch $voice, afilt+(aout*0.5)
;		  endin
...
;
;klfo	  oscil kvibenv, 0.725, 1
;kindex    expon 1, 0.2, 0.30
;
;asig1 	  oscili  kenv*iamp, kfrq, 1 
;asig2	  oscili  kenv*iamp, kfrq*1.002, 1
;asig3	  oscili  kenv*iamp, kfrq*.998, 1
;asig4	  foscil  kenv, kfrq, 1, 1, kindex
;
;amix	  = (asig1+(asig2*0.5)+(asig3*0.5)+(asig4*0.015))*6
;anoise	  rand kfrq
;afilt	  tone amix+(anoise*0.0000025*klfo), 20000
;aout	  distort	afilt*kenv,kvibenv*25, gifn
;		  outch $harmony, amix ;afilt+(aout*0.5)
;		  endin

;          instr $harmony ; more focus on tension
;
;ivibdel   = 0.714
;iamp      veloc 0, 0.25
;kfrq      cpsmidib 1.0
...
;asig2     oscili  kenv*iamp, kfrq*1.002, 1
;asig3     oscili  kenv*iamp, kfrq*.998, 1
;asig4     foscil  kenv, kfrq, 2, 2, kindex
;
;amix      = (asig1+(asig2*0.5)+(asig3*0.5)+(asig4*0.015))*6
;anoise    rand kfrq
;aout      distort    amix*kenv,kvibenv*1, gifn
;;afilt     tone amix+(anoise*0.00005*klfo), 100
;;aout      distort amix, gifn
;          outch $harmony, aout
;	 endin
